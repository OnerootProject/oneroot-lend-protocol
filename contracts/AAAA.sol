...
contract AAAAPool is Configable, BaseMintField
{
    ...
    function _mintToPool() internal {
        if(IConfig(config).mint() != address(0) && IAAAAMint(IConfig(config).mint()).take() > 0) {
            mintBalance = mintBalance.add(IAAAAMint(IConfig(config).mint()).mint());
        }
    }
    ...
    function _currentReward() internal override view returns (uint) {
        ...
        if(IConfig(config).mint() != address(0)) {
            remain = IAAAAMint(IConfig(config).mint()).take();
        }
        return remain.add(mintedShare).add(mintBalance).sub(totalShare);
    }
}